...
class RegisterRequestModel extends ValidateUserModel{
    ...
    {   var validationResult = await ValidateUserModel.validate(context,this);
        ...
        if (validationResult.meta != MetaModel.userExist()) {
            var newUser =  await context.ExecuteQuery("INSERT INTO users (first_name,last_name,email,password) VALUES (?,?,?,?)",this); 
            var oldUser =  await context.QueryFirstOrDefault("select first_name,last_name,email,password from users where first_name = ? and last_name=? and email=? and password=?",this);
